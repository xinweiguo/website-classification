...
def test(verbose=None):
    ...
    import doctest
    print('Running tests...')
    doctest.testfile(relative_filename('ngrams-test.txt'), verbose=verbose)
...
def datafile(name, sep='\t'):
    ...
    with open(name,encoding='utf8',errors='replace') as f:
        for line in f:
...
Pw  = Pdist(datafile(relative_filename('count_1w.txt')), N, avoid_long_words)
...
P2w = Pdist(datafile(relative_filename('count_2w.txt')), N)

# @memo
@cached(cache=LRUCache(maxsize=200000))
...
def combine(Pfirst, first, Premrem): 
    "Combine first and rem results into one (probability, words) pair." 
    # return Pfirst+Prem, [first]+rem 
    return Pfirst+Premrem[0], [first]+Premrem[1]
...
P3l = Pdist(datafile(relative_filename('count_3l.txt'))) 
P2l = Pdist(datafile(relative_filename('count_2l.txt'))) ## We'll need it later 
...
def correct(w): 
    ...
    c, edit = max(candidates, key=lambda ce: Pedit(ce[1]) * Pw(ce[0])) 
    return c 
...
P1edit = Pdist(datafile(relative_filename('count_1edit.txt'))) ## Probabilities of single edits 
